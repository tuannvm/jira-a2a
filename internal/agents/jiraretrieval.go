...
func (j *JiraRetrievalAgent) formatJiraComment(task *models.InfoGatheredTask) string {
	...
	if suggestion, ok := task.CollectedFields["Suggestion"]; ok {
	...
	for key, value := range task.CollectedFields {
		if key != "Suggestion" { // We already included this above
			sb.WriteString(fmt.Sprintf("- *%s:* %s\n", key, value))
		}
	}

	// Add footer
	sb.WriteString("\n_This comment was automatically generated by the A2A Information Gathering System._")
